    ...
    if (state[userId]) {
      ...
      if (s.step === 'name') {
        ...
        await saveSheet({
          ...
          telegram_username: (update.message.from.username ? '@' + update.message.from.username : ''),
        ...
        await notifyAdmin(
          ...
          e6 + ' Telegram: ' + (update.message.from.username ? '@' + update.message.from.username : '') + ' (' + userId + ')'
        );
        s.step = 'rematch';
